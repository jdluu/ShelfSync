...
pub async fn get_calibre_metadata(pool: &deadpool_sqlite::Pool) -> Result<Vec<Book>, AppError> {
    ...
    conn.interact(move |conn| -> Result<Vec<Book>, AppError> {
            ...
            conn.prepare("SELECT book, GROUP_CONCAT(format, ',') FROM data GROUP BY book")
        {
            if let Ok(mut rows) = fmt_stmt.query([]) {
                while let Ok(Some(row)) = rows.next() {
                    ...
                    {
                        formats_map
                            .insert(book_id, fmts.split(',').map(|s| s.to_string()).collect());
                    }
                }
            }
        }
        ...
        let mut desc_map: std::collections::HashMap<i64, String> = std::collections::HashMap::new();
        if let Ok(mut desc_stmt) = conn.prepare("SELECT book, text FROM comments") {
        ...
        let mut rating_map: std::collections::HashMap<i64, f64> = std::collections::HashMap::new();
        ...
        let mut lang_map: std::collections::HashMap<i64, String> = std::collections::HashMap::new();
...
mod tests {
    ...
    fn create_mock_calibre_db(path: &Path) {
        ...
        conn.execute("INSERT INTO books_tags_link (book, tag) VALUES (1, 1)", [])
            .unwrap();
        conn.execute(
            "INSERT INTO publishers (id, name) VALUES (1, 'Test Publisher')",
            [],
        )
        .unwrap();
        conn.execute(
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO comments (book, text) VALUES (1, '<p>A great book.</p>')",
            [],
        )
        .unwrap();
        conn.execute(
