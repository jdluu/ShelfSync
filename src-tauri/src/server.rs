...
async fn get_manifest(State(state): State<SharedState>) -> impl IntoResponse {
    let library_path_guard = state.library_path.lock().unwrap();
    let library_path = match &*library_path_guard {
        Some(path) => path,
        None => return (axum::http::StatusCode::SERVICE_UNAVAILABLE, "Library path not set").into_response(),
    };

    match db::get_calibre_metadata(library_path) {
...
async fn get_cover(
    ...
    // In a real scenario, we should query DB to get the book's relative path.
    // However, since db::get_calibre_metadata returns all books, we can re-query or iterate.
    // For simplicity/performance MVP, we'll re-query just this book or scan.
    // Ideally, we should add a `get_book_by_id` to `db.rs`. 
    // Let's assume we implement `db::get_book` or similar. For now, we will just fetch all and find it (inefficient but safe for MVP).
    
    let books = match db::get_calibre_metadata(&library_path) {
        Ok(b) => b,
        Err(_) => return (StatusCode::NOT_FOUND, "Library read error").into_response(),
    };

    let book = match books.into_iter().find(|b| b.id == book_id) {
        Some(b) => b,
    ...
    match File::open(cover_path).await {
        Ok(file) => {
...
mod tests {
    ...
    async fn test_manifest() {
        ...
        let json = response.json::<Vec<db::Book>>();
