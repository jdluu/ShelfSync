...
export const RoleSelection: React.FC<RoleSelectionProps> = ({ onSelect, onBrowseCatalog }) => {
  return (
      ...
      <main id="main-content" className="flex-grow flex flex-col p-4 sm:p-6 pt-8 sm:pt-12">
        <div className="container max-w-4xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
            ...
            {/* Browse Catalog (OPDS) Card - only rendered when a handler is provided */}
            {onBrowseCatalog && (
              <button
                type="button"
                onClick={onBrowseCatalog}
                className="card bg-base-100 hover:bg-base-200/50 border border-base-content/10 hover:border-accent/50 transition-all duration-300 shadow-sm hover:shadow-xl group cursor-pointer relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-accent/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <div className="card-body items-center text-center py-10 sm:py-16 gap-4 sm:gap-6 relative z-10">
                  <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-[2rem] bg-base-200/80 flex items-center justify-center group-hover:scale-[1.08] group-hover:bg-accent/10 transition-all duration-300 ring-1 ring-base-content/5 group-hover:ring-accent/20">
                    <BookOpen className="w-10 h-10 sm:w-12 sm:h-12 text-base-content/60 group-hover:text-accent transition-colors duration-300" />
                  </div>
                  <div className="space-y-3 sm:space-y-4">
                    <h2 className="text-2xl sm:text-3xl font-display font-bold tracking-tight">
                      Browse Catalog (OPDS)
                    ...
                    <p className="text-sm sm:text-base text-base-content/60 leading-relaxed max-w-xs mx-auto">
                      Download books directly from external OPDS catalogs, no host required.
                    </p>
                  </div>
                  <div className="btn btn-accent mt-2 sm:mt-4 px-8 sm:px-12 transition-all btn-sm sm:btn-md shadow-sm group-hover:shadow-md group-active:scale-95 font-medium">
                    Select Role
                  </div>
                </div>
              </button>
